# Scenario: websocket_c2_remote_control

## 1. Purpose
Validate WebSocket-based C2 communication and HTTP relay behavior in a controlled environment using mock endpoints only.

## 2. Matched Static Pattern
WebSocket connection to remote server, server-instructed fetch execution, and response exfiltration (body, headers, status) indicate C2-driven HTTP relay behavior.

## 3. Expected Flows
session_id received -> websocket connect -> server sends fetch instructions -> fetch executed -> response (headers, status, body) sent back via socket.
