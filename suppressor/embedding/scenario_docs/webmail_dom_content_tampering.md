# Scenario: webmail_dom_content_tampering

## 1. Purpose
Validate periodic DOM rewrite behavior on mock webmail content using inert dummy replacement text.

## 2. Matched Static Pattern
DOM text search and html() replacement in interval loop indicate repeated content tampering.

## 3. Expected Flows
interval tick -> locate target mock nodes -> replace with dummy text -> collect mutation evidence.
