# Scenario: ad_fraud_session_replay

## 1. Purpose
Validate Bloom filter domain targeting and session replay fetch loop behavior in a controlled environment using mock endpoints only.

## 2. Matched Static Pattern
Bloom filter binary blob download, domain hash matching, and repeated fetch execution indicate ad fraud via session replay behavior.

## 3. Expected Flows
bloom_filter_download -> domain_hash_check -> url_send_to_server -> session_id_receive -> fetch_loop_replay -> response_exfiltration.
