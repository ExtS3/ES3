# Scenario: dnr_security_header_bypass

## 1. Purpose
Validate declarativeNetRequest header modifications in a controlled environment and confirm only mock traffic paths are touched.

## 2. Matched Static Pattern
declarativeNetRequest modifyHeaders/updateSessionRules indicates CSP removal or header spoofing behavior. Additional signals include updateDynamicRules with addRules/removeRuleIds for runtime rule injection, Content-Security-Policy and Content-Security-Policy-Report-Only removal, X-Frame-Options removal to enable frame injection, operation remove with urlFilter wildcard on main_frame resource type, volumeHeaderRules storage pattern for server-pushed rule updates, Access-Control-Allow-Origin wildcard set operation for CORS bypass, and webRequest.onBeforeSendHeaders used to intercept and replace or add headers before requests are sent.

## 3. Expected Flows
install/update DNR rules -> trigger mock request -> collect request/response header modification traces.
