# Scenario: popunder_ad_tab_injection

## 1. Purpose
Validate remote config-driven background tab creation behavior in a controlled environment using mock endpoints only.

## 2. Matched Static Pattern
Server-provided newsPage value passed to chrome.tabs.create indicates silent background tab ad injection behavior.

## 3. Expected Flows
config_fetch -> newsPage_value_received -> tabs_create_background -> ad_page_loaded_silently.
