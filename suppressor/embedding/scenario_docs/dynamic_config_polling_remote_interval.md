# Scenario: dynamic_config_polling_remote_interval

## 1. Purpose
Validate install-time check-in and dynamic polling interval update logic using localhost config responses only.

## 2. Matched Static Pattern
manifest version check, alarms scheduling, sync storage interval updates, and POST check-in indicate adaptive polling control. Additional signals include dead drop resolver pattern where C2 address is parsed from a public blog or external service rather than hardcoded, and installParams serialization sent on first install check-in. Further signals include chrome.runtime.reload() used to force periodic restart ensuring accumulated data is sent, chrome.storage.local.get + chrome.storage.local.set config merge pattern where server response is merged with existing local config before re-storing, and atob double-decoding combined with JSON.parse used to unpack obfuscated remote instructions.

## 3. Expected Flows
on install -> localhost check-in -> receive mock interval -> update alarms/sync state -> periodic polling evidence.
