# Scenario: ad_frame_inline_injection

## 1. Purpose
Validate CSP removal and remote HTML payload injection behavior in a controlled environment using mock endpoints only.

## 2. Matched Static Pattern
X-Frame-Options and CSP removal via declarativeNetRequest combined with img onload eval execution and frame overlay indicates inline ad frame injection behavior.

## 3. Expected Flows
csp_remove -> html_payload_inject -> img_onload_eval -> loadScript_fetch -> frame_inject -> ad_overlay_search_results.
