# Scenario: startup_c2_backdoor_tabs_create

## 1. Purpose
Validate startup-driven remote configuration handling and tab creation behavior in a localhost-only command simulation.

## 2. Matched Static Pattern
runtime startup hooks + fetch POST + tabs.create indicates automated command-and-open behavior. Additional signal includes chrome.runtime.onInstalled listener which triggers malicious behavior immediately on first install or extension update.

## 3. Expected Flows
on startup -> localhost config fetch -> parse mock response -> create or request tab navigation event.
