# Scenario: c2_response_dom_innerhtml_injection

## 1. Purpose
Validate whether remote response text is inserted into DOM unsafely on mock pages, without executing harmful payloads.

## 2. Matched Static Pattern
innerHTML/insertAdjacentHTML with fetch response handling indicates unsanitized remote content insertion risk. Additional signals include C2 response fields such as result.rating and result.protxt being written directly to innerHTML without sanitization, and classList.remove() used to unhide injected elements after insertion.

## 3. Expected Flows
fetch mock content -> apply DOM write path -> observe mutation and injection markers.
