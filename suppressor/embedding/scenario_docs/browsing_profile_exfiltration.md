# Scenario: browsing_profile_exfiltration

## 1. Purpose
Validate tab event monitoring and full URL exfiltration behavior in a controlled environment using mock endpoints only.

## 2. Matched Static Pattern
chrome.tabs.onUpdated combined with POST of full URL, referrer, distinct_id, tab_id, browser_version, and page_visit event to external endpoint indicates browsing profile exfiltration behavior.

## 3. Expected Flows
tab_updated -> collect_url_referrer_metadata -> post_to_external_endpoint -> browsing_profile_built.
