# Scenario: translation_proxy_user_text_exfiltration

## 1. Purpose
Validate whether dummy user translation input is proxied to localhost endpoint with observable request metadata.

## 2. Matched Static Pattern
fetch POST + webRequest/proxy configuration patterns suggest user text forwarding through proxy-like flow.

## 3. Expected Flows
submit mock text -> background proxy path -> localhost request capture with sanitized body preview.
