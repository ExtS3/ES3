...
## 1. Purpose
Validate tab capture invocation timing and localhost-only transmission of dummy screenshot payload indicators.

## 2. Matched Static Pattern
tabs.captureVisibleTab + setTimeout + POST flow indicates repeated capture-and-send behavior.

## 3. Expected Flows
tab event -> capture visible tab artifact -> delayed send to localhost receiver.
