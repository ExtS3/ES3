# ExtAnalysis — 프로젝트 통합 방식

> **이 디렉토리는 외부 오픈소스 라이브러리 [ExtAnalysis v1.0.5](https://github.com/Tuhinshubhra/ExtAnalysis)입니다.**  
> 본 프로젝트에서는 독립 실행 도구가 아닌 **패키지 수집기 라이브러리**로 활용합니다.

## 이 프로젝트에서 사용하는 방식

역할 분리:
- **ExtAnalysis (이 디렉토리)**: ZIP 해제 → `manifest.json` 파싱 → 파일 목록/URL/`source.json` 생성 → `db/permissions.json` 권한 참조 데이터 제공
- **우리 스캐너 (`backend/scanners/`)**: ExtAnalysis가 모은 데이터를 기반으로 실제 위험도 판정

통합 진입점: `backend/extanalysis_integration.py`

주요 변경점 (integration에서 우회):
- ExtAnalysis 내부의 VirusTotal / GeoIP / DNS 조회 비활성화
- `manifest.json`이 ZIP 최상위가 아닌 하위 디렉토리에 있어도 자동 탐색
- `reputation_targets`는 정적 스캐너가 재분류한 것만 후속 평판 조회에 사용

활용하는 출력물:
- `lab/{analysis_id}/source.json` — JS 소스 코드 맵 (정적 스캐너 입력)
- `db/permissions.json` — 권한 위험도 참조 데이터 (`manifest_permission_scan` 참조용)

자세한 내용은 [backend/README.md](../backend/README.md)와 [STATIC_ANALYSIS_GUIDE.md](../STATIC_ANALYSIS_GUIDE.md)를 참고하세요.

---

<!-- 아래는 원본 ExtAnalysis 라이브러리 README입니다 -->
